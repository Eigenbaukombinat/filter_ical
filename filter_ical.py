...
kalender_url = 'kalender_url'
username = ''
password = ''

icscontent = requests.get(kalender_url, auth=(username, password)).text

with open(f'{output_dir}/all.ics', 'w') as alleics:
...
with open(f'{output_dir}/public.ics', 'w') as pubics:
    pubics.write('\n'.join(public_ics))
